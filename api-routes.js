...
router
  ...
  .get(async (req, res) => {
    try {
      const [invItem] = await db.query(`SELECT * FROM inventory`)
      res.json(invItem)
    } catch (err) {
  ...
  .post(async (req, res) => {
    try {
      ...
      res.status(500).send('Error creating inventory item: ' + err.message)
    }
  })
...
router
  ...
  .get(async (req, res) => {
    try {
      const [inventoryId] = await db.query(`SELECT * FROM inventory WHERE id = ?`,
        req.params.id)

      if (!inventoryId[0]) {
        return res.status(404).send('Error finding inventory item')
      }
      res.json(inventoryId[0])
    } catch (err) {
      res.status(500).send('Error creating inventory item: ' + err)
    }
  })
  ...
  .put(async (req, res) => {
    try {
      ...
      res.status(204).send('Inventory item successfully updated')
    } catch (err) {
      res.status(500).send('Error updating inventory item: ' + err.message)
    }
  ...
  .delete(async (req, res) => {
    try {
      ...
      res.status(204).send('Inventory item successfully deleted')
    } catch (err) {
